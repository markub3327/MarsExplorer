...
class Lidar:
    ...
    def __init__(
        self, r=np.inf, fov=[0, 2 * np.pi], channels=8, resolution=200, map=None
    ):
    ...
    def scan(self, position, map=None, resolution=None, dtype=np.int64):
        ...
        p2 = np.stack(
            (
                p1[0] + self.maxRange * np.cos(self.thetas),
                p1[1] + self.maxRange * np.sin(self.thetas),
            ),
            axis=1,
        )
        ...
        for channel in range(self.channels):
            ...
            if obstacles_idx.size > 0:
                ...
                distances = np.linalg.norm(
                    p1_repeated - rays_channel[obstacles_idx], axis=1
                )
    ...
    def ranges_to_idx(self, position, resolution=None):
        ...
        # visible
        extended_r = self.ranges + 0.5
        extended_r = np.clip(extended_r, 0.0, self.maxRange)

        p2 = np.stack(
            (
                p1[0] + extended_r * np.cos(self.thetas),
                p1[1] + extended_r * np.sin(self.thetas),
            ),
            axis=1,
        )
