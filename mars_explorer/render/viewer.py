...
class Viewer:
    ...
    def load_data(self):
        ...
        self.drone_img = pg.transform.scale(
            self.drone_img, (self.TILESIZE, self.TILESIZE)
        )

        self.obstacle_img = pg.image.load(self.conf["obstacle_img"]).convert_alpha()
        self.obstacle_img = pg.transform.scale(
            self.obstacle_img, (self.TILESIZE, self.TILESIZE)
        )

        self.bck_img = pg.image.load(self.conf["background_img"]).convert_alpha()
        self.bck_img = pg.transform.scale(
            self.bck_img, (self.conf["height"], self.conf["width"])
        )
    ...
    def render_fog_camera(self):
        ...
        cameraX = (self.env.x + 0.5) * self.TILESIZE
        cameraY = (self.env.y + 0.5) * self.TILESIZE
    ...
    def render_fog_explored(self):
        ...
        explored_idx = np.where(self.env.outputMap > 0.0)
    ...
    def draw_traceline(self):
        ...
        if self.env.timeStep > 2:
            for step in range(len(self.env.drone_trajectory) - 1):
                ...
                currentX = (
                    self.env.drone_trajectory[-1 - step][0] * self.TILESIZE
                    + 0.5 * self.TILESIZE
                )
                currentY = (
                    self.env.drone_trajectory[-1 - step][1] * self.TILESIZE
                    + 0.5 * self.TILESIZE
                )
                prevX = (
                    self.env.drone_trajectory[-2 - step][0] * self.TILESIZE
                    + 0.5 * self.TILESIZE
                )
                prevY = (
                    self.env.drone_trajectory[-2 - step][1] * self.TILESIZE
                    + 0.5 * self.TILESIZE
                )

                pg.draw.line(
                    self.screen, self.BLUE, (currentX, currentY), (prevX, prevY)
                )
